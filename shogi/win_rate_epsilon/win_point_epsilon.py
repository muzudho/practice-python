...
def main():
    ...
    for black_win_rounds in range(1,all_rounds):
        ...
        #                       2 * (1 - black_win_rate)
        #
        # 先手の１勝時に移動する点　＝　先手の１勝の値打ち　×　(2 / （先手の１勝の値打ち ＋ 後手の１勝の値打ち）)
        # 後手の１勝時に移動する点　＝　後手の１勝の値打ち　×　(2 / （先手の１勝の値打ち ＋ 後手の１勝の値打ち）)
        #
        # ※ 先手と後手の合計が 2勝 になるように引き延ばす
        # 先手の１勝の加点　＝　先手の１勝時に移動する点 / 2
        # 後手の１敗の減点　＝　－（先手の１勝時に移動する点 / 2）
        #
        # 後手の１勝の加点　＝　後手の１勝時に移動する点 / 2
        # 先手の１敗の減点　＝　－（後手の１勝時に移動する点 / 2）
        #
        ...
        black_win_moving_point = black_win_point_a * ratio   # 先手の１勝時に移動する点
        white_win_moving_point = white_win_point_a * ratio   # 後手の１勝時に移動する点
...
先手勝率: {black_win_rate} のとき、
    ...
    先手の１勝時に移動する点: {black_win_moving_point:20.16f}
    後手の１勝時に移動する点: {white_win_moving_point:20.16f}
